...
const CaseRoulette: React.FC<CaseRouletteProps> = ({ 
  ...
  const handleImmediateSell = async (item: CaseItem) => {
    try {
      ...
      setTimeout(() => {
        console.log('💰 ПРОДАЖА: Автоматически закрываем окно после продажи');
        handleClose();
      }, 2000);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 bg-[#121212]/90 backdrop-blur-xl z-50 flex items-center justify-center p-2 sm:p-4">
                  ...
                  <Button
                    onClick={async () => {
                      if (winnerItem && !soldOrAdded) {
                        ...
                        // Продаем предмет сразу (setSoldOrAdded устанавливается внутри handleImmediateSell)
                        await handleImmediateSell(winnerItem);
                        
                        console.log('🔄 КНОПКА ПРОДАТЬ: Продажа завершена');
                      } else {
