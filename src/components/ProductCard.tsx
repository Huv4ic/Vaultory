...
const ProductCard = ({ id, name, price, originalPrice, image, category, rating, sales, isInCart, onAddToCart, onDetails }: ProductCardProps) => {
  ...
  return (
    <div className="rounded-2xl shadow-xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-6 flex flex-col items-center text-center transition-transform duration-200 hover:scale-105">
      <img src={image} alt={name} className="w-24 h-24 object-contain mx-auto mb-4 rounded-xl shadow-lg bg-white/10" />
      <div className="text-xl font-bold mb-1 text-white drop-shadow-lg animate-fade-in">{name}</div>
      <div className="text-base text-gray-100 mb-2 animate-fade-in">{category}</div>
      {originalPrice && (
        <div className="mb-2">
