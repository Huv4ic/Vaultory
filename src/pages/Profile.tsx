...
const Profile = () => {
  ...
  const handleSellItem = async (index: number) => {
    ...
    setSellingItem(null);
         ...
         description: `${t("Вы получили")} ${soldAmount}₴ ${t("за продажу предмета")}`,
       });
  };
  ...
  const handleWithdrawItem = (index: number) => {
    withdrawItem(index);
       ...
       description: t("Предмет будет обработан и выдан в ближайшее время"),
     });
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto bg-gray-800/90 text-white rounded-2xl shadow-2xl p-8 mt-8 animate-fade-in flex flex-col items-center border border-gray-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8">
               ...
               <span className="text-gray-400 text-base mb-1">{t("Потрачено за всё время")}</span>
               <span className="text-2xl font-extrabold text-green-400">{spent}₴</span>
             </div>
             <div className="bg-gray-800 rounded-xl p-6 flex flex-col items-center min-h-[110px] min-w-[140px]">
               <span className="text-gray-400 text-base mb-1">{t("Открыто кейсов")}</span>
               <span className="text-2xl font-extrabold text-blue-400">{casesOpened}</span>
             </div>
             <div className="bg-gray-800 rounded-xl p-6 flex flex-col items-center min-h-[140px] min-w-[140px]">
               <span className="text-gray-400 text-base mb-1">{t("Куплено товаров")}</span>
        ...
        <div className="max-w-4xl mx-auto mt-12">
          <Card className="bg-gray-800/50 backdrop-blur-sm border-gray-700/50">
            <CardHeader>
                 ...
                 <span>{t("Инвентарь")}</span>
               </CardTitle>
              <div className="flex items-center gap-4 mt-4">
                                 <span className="bg-gray-700 text-white px-3 py-1 rounded-full text-sm">{items.length} {t("предметов")}</span>
                 <span className="text-green-300 font-bold">{t("Сумма")}: {getTotalValue()}₴</span>
                 <span className="text-blue-400 font-bold">{t("Открыто кейсов")}: {casesOpened}</span>
              </div>
            </CardHeader>
            <CardContent>
              {items.length === 0 ? (
                <div className="text-center py-12">
                  <Package className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                                     <h3 className="text-xl font-semibold text-white mb-2">{t("Инвентарь пуст")}</h3>
                   <p className="text-gray-400 mb-6">
                     {t("Откройте кейсы, чтобы получить предметы в инвентарь")}
                   </p>
                   <Link to="/cases">
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2 mt-2">
                          ...
                          >
                                                         {item.status === 'withdrawn' ? t("Получено") : (
                               <>
